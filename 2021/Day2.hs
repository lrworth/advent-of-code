...
data Input' = Input' Natural Natural Char String
  deriving (Show)

type Parsec = P.Parsec Void String

inputsP :: Parsec [Input]
inputsP = P.many inputP
...
inputP =
  ( Input
      <$> (P.decimal <* P.char '-')
      <*> (P.decimal <* P.char ' ')
      <*> (P.letterChar <* P.string ": ")
      <*> P.many P.alphaNumChar
  )
    <* P.newline

input'sP :: Parsec [Input']
input'sP = P.many input'P

input'P :: Parsec Input'
input'P =
  ( Input'
      <$> (P.decimal <* P.char '-')
      <*> (P.decimal <* P.char ' ')
...
valid :: Input -> Bool
valid (Input min max letter password) = let occs = fromIntegral $ length (filter (== letter) password) in min <= occs && occs <= max

valid' :: Input' -> Bool
valid' (Input' pos1 pos2 letter password)
  | let l = fromIntegral $ length password in pos1 <= l && pos2 <= l =
    let p1m = (password !! (fromIntegral $ pos1 - 1) == letter)
        p2m = (password !! (fromIntegral (pos2 - 1)) == letter)
     in (p1m || p2m) && not (p1m && p2m)
  | otherwise = False
