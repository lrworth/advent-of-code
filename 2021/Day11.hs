...
type Board = A.Array (Int, Int) State
...
parse :: String -> Board
parse = listsToArray . (fmap . fmap) parseState . lines
  where
    listsToArray :: [[State]] -> Board
    listsToArray sss =
      let height = length sss
          width = length (head sss)
       in A.listArray ((1, 1), (height, width)) . concat $ sss
...
safeLookup :: Board -> (Int, Int) -> Maybe State
safeLookup b (y, x) =
  let ((ly, lx), (hy, hx)) = bounds b
   in if lx <= x && x <= hx && ly <= y && y <= hy
        then Just $ b ! (y, x)
        else Nothing

surrounds :: Board -> (Int, Int) -> [State]
surrounds b (y, x) =
  Maybe.mapMaybe
    (safeLookup b)
    [ (y, x - 1),
...
step :: Board -> Board
step b = (b //) . fmap (\i -> (i, newState i)) $ indices b
  where
    ...
    newState i =
      let seat = b ! i
          surr = surrounds b i
       in if
              | seat == Empty && L.find (== Occupied) surr == Nothing -> Occupied
              | seat == Occupied && length (L.filter (== Occupied) surr) >= 4 -> Empty
              | otherwise -> seat

stabilise :: Board -> Maybe Board
stabilise = fmap fst . L.find (\(a, b) -> a == b) . uncurry zip . (id &&& tail) . iterate step

solveA :: String -> Maybe Int
solveA = pure . length . L.filter (== Occupied) . A.elems <=< stabilise . parse
...
numOccupiedVisible :: Board -> (Int, Int) -> Int
numOccupiedVisible b i =
  let occupiedVisible off =
        maybe False (== Occupied)
          . Maybe.listToMaybe
          . L.dropWhile (== Floor)
          . Maybe.catMaybes
          . tail
          . L.takeWhile Maybe.isJust
          . fmap (safeLookup b)
          . iterate off
          $ i
   in length
...
step' b = (b //) . fmap (\i -> (i, newState i)) $ indices b
  where
    ...
    newState i =
      let seat = b ! i
          occs = numOccupiedVisible b i
       in if
              | seat == Empty && occs == 0 -> Occupied
              | seat == Occupied && occs >= 5 -> Empty
              | otherwise -> seat
...
solveB = pure . length . L.filter (== Occupied) . A.elems <=< stabilise' . parse
