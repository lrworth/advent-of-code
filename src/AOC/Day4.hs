...
inputPassports :: String -> [[String]]
inputPassports = fmap (sort . words . intercalate " ") . wordsBy (== []) . lines
...
fieldValid f = case second tail . span (/= ':') $ f of
  ("byr", num) -> length num == 4 && (let x = read num in 1920 <= x && x <= 2002)
  ("iyr", num) -> length num == 4 && (let x = read num in 2010 <= x && x <= 2020)
  ("eyr", num) -> length num == 4 && (let x = read num in 2010 <= x && x <= 2030)
  ("hgt", num) ->
    let (s, unit) = splitAt (length num - 2) num
        x = read s
     in case unit of
          "cm" -> 150 <= x && x <= 193
          "in" -> 59 <= x && x <= 76
          _ -> False
  ("hcl", '#' : num) -> length num == 6 && all (`elem` "0123456789abcdef") num
  ("ecl", num) -> num `elem` ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]
  ("pid", num) -> length num == 9 && all (`elem` "0123456789") num
  _ -> False
...
input :: String
input =
  [r|ecl:#eef340 eyr:2023 hcl:#c0946f pid:244684338 iyr:2020 cid:57 byr:1969 hgt:152cm
