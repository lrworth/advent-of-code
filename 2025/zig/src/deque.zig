//! Taken from https://ziglang.org/documentation/master/std/#src/std/deque.zig
//! Once Zig >0.15 is released, use the std version of this.
...
pub fn Deque(comptime T: type) type {
    return struct {
        ...
        pub fn ensureTotalCapacity(deque: *Self, gpa: Allocator, new_capacity: usize) Allocator.Error!void {
            ...
            const minimum = new_capacity;
            const init_capacity: comptime_int = @max(1, std.atomic.cache_line / @sizeOf(T));

            const growCapacity = minimum +| (minimum / 2 + init_capacity);

            return deque.ensureTotalCapacityPrecise(gpa, growCapacity);
        }
